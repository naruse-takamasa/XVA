...
MonteCarloEngine::MonteCarloEngine(const MCConfig& config, const HullWhiteModel& model)
    : config_(config), model_(model) {}

// ── Single-threaded block simulation ─────────────────────────────────────────
void MonteCarloEngine::simulateBlock(std::size_t pathStart, std::size_t pathEnd,
                                     const std::vector<InterestRateSwap>& swaps,
                                     const TimeGrid& grid, Matrix& pathValues,
                                     unsigned seed) const {
    ...
    for (std::size_t p = 0; p < blockSize; ++p) {
        for (std::size_t i = 0; i < N; ++i) {
            ...
            Real rt = rPaths[p][i];
            Real portNPV = 0.0;
            for (auto& swap : swaps) portNPV += swap.npv(t, rt, model_);
...
SimulationResult MonteCarloEngine::runPortfolio(const std::vector<InterestRateSwap>& swaps,
                                                const TimeGrid& grid) const {
    LOG_INFO("MC simulation: ", config_.numPaths, " paths, ", grid.size(), " time steps, ",
             config_.numThreads, " threads");

    SimulationResult result;
    result.timeGrid = grid;
    result.numPaths = config_.numPaths;
    result.pathValues.assign(config_.numPaths, std::vector<Real>(grid.size(), 0.0));
...
SimulationResult MonteCarloEngine::run(const InterestRateSwap& swap, const TimeGrid& grid) const {
...
void MonteCarloEngine::computeExposureProfiles(SimulationResult& result) const {
    ...
    for (std::size_t i = 0; i < N; ++i) {
        for (std::size_t p = 0; p < P; ++p) col[p] = result.pathValues[p][i];
