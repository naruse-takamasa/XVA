...
FVACalculator::FVACalculator(Real spread) : borrowingSpread_(spread), lendingSpread_(spread) {}
...
Real FVACalculator::computeFCA(const SimulationResult& result, const YieldCurve& disc) const {
    Real fca = 0.0;
...
Real FVACalculator::computeFBA(const SimulationResult& result, const YieldCurve& disc) const {
    Real fba = 0.0;
    std::size_t N = result.timeGrid.size();
    for (std::size_t i = 1; i < N; ++i) {
        Real dt = result.timeGrid[i] - result.timeGrid[i - 1];
...
Real FVACalculator::compute(const SimulationResult& result, const YieldCurve& disc) const {
